...
const start = computed(() => {
  return props.blockMap
    ? block.value?.value?.format?.list_start_index ||
        getListNumber(block.value.value.id, props.blockMap)
    : 0;
});
const isTopLevel = computed(
  () => type.value != props.blockMap?.[block.value.value.parent_id].value.type,
);
</script>
...
<template>
  ...
  >
    <li><NotionTextRenderer :text="title" v-bind="pass" /></li>
    <NotionNestedList
      :class="[level == 1 ? 'notion-list-flat' : '']"
  ...
  <span v-else>
    <li :class="[level != 1 ? 'notion-list-indent' : '']">
